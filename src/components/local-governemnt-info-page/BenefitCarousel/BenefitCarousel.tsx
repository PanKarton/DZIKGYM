...
const circleButtonStyle =
  "w-20 aspect-square border-2 border-(--blue) rounded-full flex-center text-(--blue) hover:bg-(--blue) hover:text-(--primary) transition cursor-pointer";
      ...
      <div className="content-max-width">
        ...
        <div className="relative w-full aspect-[4/1] flex items-center justify-center">
          {benefits.map((item, i) => (
            <div
              key={i}
              className={`absolute transition-all duration-500 ease-in-out bg-white rounded-3xl shadow-2xl py-16 px-8 border-(--offWhite) border-2 flex-center gap-8 w-[65%] aspect-[870/400] transform ${getPositionClass(i)}`}
            >
              ...
              <div className="text-left">
                <h3 className="text-(--blue) font-black italic mb-4 text-[2rem]">
                  {item.title}
                </h3>
                <p className="text-sm text-(--secondary) leading-snug font-normal">
                  {item.description}
                </p>
              </div>
            </div>
        ...
        <div className="flex items-center justify-center gap-24 mt-24">
          ...
          <span className="text-3xl font-medium text-[var(--gray)]">
            {index + 1}/{total}
          </span>
